...
| 27 | **Pravki-no-deposit-waived-marker** (commits `3796531` + `5bfeec7` + `241115f` + `4dc2b08` + `9c32d6f`) | ✅ Готово к деплою (коммит `9c32d6f` — основное закрытие дыры для PAUSED-юзеров) | Закрыта финансовая дыра «юзер на паузе без денег → пополнил → catch за прошедший день → деньги списываются повторно». Решение (по коммитам): (1) **`5bfeec7`**: `apply_window_expired` при `deposit == 0` пишет маркерную `Penalty(reason=WAIVED_UNABLE_TO_PAY, amount=0)` для редкого ACTIVE+deposit=0; (2) **`241115f`**: `apply_catch` idempotency расширена на ВСЕ reason'ы — за день есть любая Penalty → catch отвергается (`penalty_already_processed`). Бонус — закрыта ещё одна дыра: прямой POST /catch поверх существующего `WINDOW_CLOSED_NO_CATCH` теперь корректно отвергается (UNIQUE `uq_penalty_per_day_reason` не срабатывал из-за отличающегося reason); (3) **`4dc2b08`**: рефактор — удалён мёртвый дубль `apply_window_expired` в `close_catch_window`; (4) **`9c32d6f` (коммит A, основное)**: rename enum → `WAIVED_UNABLE_TO_PAY` (Wide-семантика: покрывает и deposit=0, и частичный deposit < penalty); новый метод `PenaltyService.mark_waived_unable_to_pay` для `status=PAUSED` юзеров (основной путь, который раньше skip'ался фильтром `if status != ACTIVE: continue`); `close_catch_window._close_for_habit` теперь ветвится по статусу и инкрементит счётчик `waived` в summary (observability без ручного SQL). `can_catch` в /members уже использовал `ids_with_any_penalty_today` — defense-in-depth на сервисе теперь полный. **Никаких миграций** (String(64) без CHECK constraint, и `reason` — VARCHAR, не ENUM; 0 строк в проде с этим reason на момент коммита). 11 backend-тестов: 5 новых для `mark_waived_unable_to_pay` + 6 предыдущих (2 `apply_window_expired_*`, 4 `apply_catch_*`). Backend suite: 404 passed (+11 vs baseline 393), 16 pre-existing failed (admin/photo). Подробности — `Pravki-no-deposit-waived-marker.md`. |
