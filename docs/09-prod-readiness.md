...
## 1. Текущая стадия: **~95% готовности к soft-launch**
...
#### Шаг 1. `/setdomain` в BotFather
- **Что**: Открыть `@BotFather` в Telegram → `/setdomain` → выбрать бота → указать `app.prideclub.fun`
- **Без этого**: Mini App не откроется кнопкой в боте (только прямая ссылка)
- **Время**: 1 минута
- **Ответственный**: Дмитрий

### 2.2 🟡 Желательно до soft-launch

#### Шаг 2. Бэкапы PostgreSQL
...
#### Шаг 4. Перенос PostgreSQL в Selectel managed
...
#### Шаг 5. Rate-limit на /api/v1/* в nginx (опционально)
...
#### Шаг 8. ФЗ-152 compliance — финальная
...
⬜ /setdomain в BotFather (1 минута)
...
| Фаза | Задачи | Время |
|------|--------|-------|
| **Готово к soft-launch** | Бэкапы (опционально) + /setdomain в BotFather | 5 минут + 30 минут |
...
**Итого до soft-launch (10-50 человек)**: 5 минут после того как Дмитрий сделает `/setdomain` в BotFather. **Бэкенд полностью готов.**
...
**Бэкенд готов к soft-launch.** 95% работы сделано: код, тесты, прод-деплой, HTTPS, rate-limit, JSON-логи, E2E через Celery. Единственное что блокирует запуск Mini App в Telegram — одна команда `/setdomain` в `@BotFather` (1 минута).
