          ...
          {items.map((h) => (
            <li key={h.id}>
              <button
                ...
                className="block w-full rounded-card border border-white/5 bg-surface p-4 text-left shadow-card transition hover:border-primary/30"
              >
                <h2 className="text-base font-semibold text-text">{h.title}</h2>
                {h.description && (
                  <p className="mt-1 line-clamp-3 text-xs text-muted">{h.description}</p>
                )}
                <div className="mt-2 flex items-center justify-between text-xs">
                  <span className="text-muted">Окно: {h.checkin_window_start}–{h.checkin_window_end}</span>
