...
> **⚠️ SUPERSEDED v2 от 2026-08-21. Использовать `TZ_kharakteristiki_personazha_v2.md` в этой же папке.**
>
> Версия v3.0 ниже (22.07.2026) описывает per-club характеристики, `icon_url`, статусы из 4 ступеней (Новичок/Практик/Мастер/Легенда) и текст «Отказался расти дальше» —
> **всё это НЕ актуально** в свете продуктового решения от 21.08.2026.
>
> Главные отличия v2:
> - **Глобальные статы** (per `user_id + stat_definition_id`), не per-club.
> - **Справочник `stat_definitions`** с 8 каноническими (intelligence/strength/endurance/balance/energy/focus/creativity/connections) — никакого свободного текста.
> - **5 статусов** с emoji `icon VARCHAR(16)`: 🐣 На старте → 🌊 В потоке → ⚡ На волне → 🔥 В форме → 🐺 Режим зверя (пороги 0/30/100/300/700).
> - **Freeze условие**: 30 дней без чек-ина в **любом** клубе с этой stat, текст «нет чек-инов более 30 дней».
> - **Admin API**: только `stat_definition_id` (без legacy fallback на `stat_name`).
>
> Документ ниже сохранён для истории и как baseline для сравнения.

---

Версия: 3.0 (SUPERSEDED — см. `TZ_kharakteristiki_personazha_v2.md` от 2026-08-21)
Дата: 22.07.2026 14:20 CEST (snapshot устарел 21.08.2026)
Базируется на: `docs/01-concept.md`, `docs/06-data-model.md`, `docs/04-code-standards.md`, `AGENTS.md`

> **Принцип (v3.0, не актуален):** фича встраивается в существующую модель данных, а не переписывает её.
